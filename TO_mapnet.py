...
def stress_and_compliance32(x0,fx0,fy0):
    nelx = 32
    nely = 32
...
def filtering(f,H,Hs):
    filtered_value=sparse.csr_matrix.dot(H,f)/Hs
    return np.asarray(filtered_value)

def mapnetoverlap_pred(dens,fx,fy):
    coarse_dens = ax.coarse_arr(dens, 16)
    coarse_fx = ax.coarse_arr(fx, 16)
    coarse_fy = ax.coarse_arr(fy, 16)
    Xigem_e_dummy, compliance_dummy, coarse_comp = stress_and_compliance32(coarse_dens,coarse_fx,coarse_fy)
    coarse_frag_comp = ax.fragmentize_overlap_more(coarse_comp.reshape(32,32).T, 4, 4)
    fine_frag_dens = ax.fragmentize_overlap_more(dens, 64, 64)
    fine_frag_comp = model.predict([1/1e-4*coarse_frag_comp.reshape(-1,4,4,1), fine_frag_dens.reshape(-1,64,64,1)])
    fine_comp_todefrag = fine_frag_comp.reshape(29*29,64,64)
    fine_comp_defrag = ax.defrag_overlap_more(fine_comp_todefrag, 29, 29)
    fine_comp = 1e-6*fine_comp_defrag.reshape(512,512).T
    compliance_e = np.maximum(fine_comp,0)
    print('cmp=',np.sum(compliance_e))
    return np.sum(compliance_e),compliance_e.reshape(1,512*512)
...
def optimization(nelx,nely,fx0,fy0,Vol0,case_num):
    ...
    Pd_filtered=np.zeros((iter_num,eleNum))
    rmin=16
    ...
    xphy_reshape = xphy.reshape(nelx,nely).T
    xphy_reshape[0:nelx//2,0:nely//2] = 0
    xphy = xphy.reshape(nelx*nely,1)
    time_start = time.time()
    compliance,compliance_e=mapnetoverlap_pred(xphy.reshape(nelx,nely).T,fx0,fy0)
    ...
    fi2=open('opt_result/fx'+str(case_num)+'_'+str(Vol0)+'.csv','ab')
    np.savetxt(fi2,fx0.reshape(1,nelx*nely),fmt='%e',delimiter=',')
    fi2.close()
    ...
    for itr in range(iter_num-1):
          ...
          comp_fake = np.array(compliance_e).reshape(nelx,nely)
          comp_fake[0:nelx//2,0:nely//2]=0
          compliance_e = comp_fake.reshape(-1)
          compliance_e=compliance_e+(fx0.T.reshape(-1)==1)*(-compliance_e+np.max(compliance_e)) 
            
          Pd_filtered[itr,:]=-filtering(xphy*((compliance_e/compliance).reshape(-1,1)),H,Hs).reshape(-1,)
          if itr==0:
...
model.load_weights(savefileid)

wd=32
gap=64
optimization_timestart = time.time()
for i in range(1):
    for vo in range(1):
        flagy=gap*(i)+256+128
        fx0=np.zeros((nely,nelx))
...
print('optimization completed at: {}'.format(time.time()-optimization_timestart))
